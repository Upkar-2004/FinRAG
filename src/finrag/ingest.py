...
Every chunk carries (doc_name, company, page) metadata so later stages can
...
def extract_pages(pdf_path) -> list[dict]:
    """Return one dict per page: {"page": int, "text": str}.

    Page numbers are 0-indexed to match the `gold_page` field in the eval set.
    """
    pages = []
